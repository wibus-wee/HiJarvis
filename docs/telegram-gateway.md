...
- 一个 Telegram chat/topic 对应某个 Jarvis entity 在 Telegram 上的局部 thread；
...
6. 先把消息路由到当前 Telegram identity，再映射到该 identity 绑定 entity 的局部本地 thread
...
- `thread`：该 identity 在某个 Telegram chat/topic 上的局部持续线程
...
Jar 本地 thread id 会进一步转成带 identity 前缀的 filesystem-safe 形态：
...
- 同一个 Telegram identity 的一个 chat 对应一个 thread
- 群聊 topic 可以独立维护上下文
- 不同 Telegram bots 即使进入同一个群聊坐标，也不会共享 thread
...
- 进程重启后，queue 状态会丢失
- 但 `.jar/threads` 里的 thread/lane 历史仍会保留
...
5. 对 bot 的上一条消息 reply，确认可以继续同一个 chat/topic thread
6. 重启服务后重新发消息，确认 thread 历史仍可继续使用
